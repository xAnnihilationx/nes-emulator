     ...
     `(progn (incf PC)
	     (let ((address #x0000)
		   ...
		   (progn
		     ...
		     (setf (ldb (byte 8 8) actual )(aref cpu-memory(+ 1 (logand #xFF00 address))))
		     )
		   (progn
		     (setf (ldb (byte 8 0) actual )(aref cpu-memory address))
...
(defmacro subtract-with-carry (address-mode)
  (case address-mode    
	...
	(let ((result (+ AC (logxor  ,memory-value #xff) (if carry-flag 1 0) )))  
	  (setf AC (ldb (byte 8 0) result))
	  (when (> result #xff)
	    (setf carry-flag nil)))
	...
	(incf cpu-cycles 4)
	(set-nz-flags AC)
	(when debug-output (format t "SBC $~X,X ~%" (aref cpu-memory (- PC 1))))	
	)))
    (:absolute
     (let ((memory-value  '(aref cpu-memory (cpu-mirror-adjust address))))       
     `(progn
	...
	(let ((address #x0000))
	  ...
	  (let ((result (+ AC (logxor (aref cpu-memory (cpu-mirror-adjust (+ address RX))) #xff) (if carry-flag 1 0) )))  
	    (setf AC (ldb (byte 8 0) result))
	    (when (> result #xff)
	      (setf carry-flag nil)))
	...
	(let ((address #x0000))
	  ...
	  (incf PC)
	   ...
	   (let ((result (+ AC (logxor (aref cpu-memory (cpu-mirror-adjust (+ address RY))) #xff) (if carry-flag 1 0) )))  
	    (setf AC (ldb (byte 8 0) result))
	    (when (> result #xff)
	      (setf carry-flag nil)))
    ...
    (:index-indirect
	 ...
	 (incf PC)
	 (let ((result (+ AC (logxor (aref cpu-memory (cpu-mirror-adjust address) ) #xff) (if carry-flag 1 0) )))  
	    (setf AC (ldb (byte 8 0) result))
	    (when (> result #xff)
	      (setf carry-flag nil)))
	 ...
	 ;; 3. cpu is wating on a vblink
	   (let* ((start-time (get-internal-real-time))
		 ...
		 ((or (>= loop-time end-time)
	       ...
	       (when debug-output (format t "CYCLE= ~5,'0D PC = ~4X P=~2X X=~2X Y=~2X AC=~2X SP=~2X "
					  cpu-cycles PC (status-register) RX RY AC SP))

	       (when (minusp SP) (error "Stack is fuckerd")) 
		 ...
		 (#x94 (store-register RY :zero-page-y "STY"))
